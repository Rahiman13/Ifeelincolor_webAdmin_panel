...
const CardHeader = styled(Box)({
  ...
  '& .plan-name': {
    ...
    paddingRight: '80px', // Make space for action buttons
  },
  '& .plan-price': {
    fontSize: '2rem',
    fontWeight: 800,
...
const PlansManagement = () => {
  ...
  const handleSavePlan = async (planData) => {
    ...
    try {
      ...
      // Determine the correct URL and method based on whether we're creating or updating
      if (planData.id) {
        ...
        // Create new plan
        url = 'https://rough-1-gcic.onrender.com/api/admin/portal-plans';
        method = 'post';
      }

      console.log(`${planData.id ? 'Updating' : 'Creating'} plan. URL:`, url);
      ...
      if (response.data.status === 'success') {
        if (planData.id) {
          setPlans(plans.map(plan => 
            plan.id === planData.id ? { ...plan, ...response.data.body } : plan
          ));
  ...
  const [formData, setFormData] = useState(() => {
    ...
    if (plan) {
      return {
        ...plan,
        startDate: today,
        endDate: calculateEndDate(today, plan.validity),
...
const ClinicianPlanCard = ({ plan, onEdit, onDelete, onStatusChange }) => {
  ...
  return (
    <StyledCard>
      <StyledCardContent sx={{ flexGrow: 1, position: 'relative', zIndex: 1 }}>
