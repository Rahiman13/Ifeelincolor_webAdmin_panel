...
const Sidebar = () => {
  ...
  // Get the user role from session storage and handle undefined or null cases
  const role = sessionStorage.getItem('role') || ''; // Fetch the user role from session storage
  ...
  return (
    <nav className="sidebar sidebar-offcanvas" id="sidebar">
      <ul className="nav">
        ...
        {role === 'organization' && (
          <li className={isPathActive('/packages/view') ? 'nav-item active' : 'nav-item'}>
            <Link className="nav-link" to="/packages/view">
              <span className="menu-title">Plan Management</span>
              <i className="mdi mdi-clipboard-list-outline menu-icon"></i>
            </Link>
          </li>
        )}
