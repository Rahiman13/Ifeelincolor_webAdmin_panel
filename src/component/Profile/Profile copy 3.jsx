      ...
      try {
        if (role === 'organization') {
          ...
          if (managersResponse.data.status === 'success' && cliniciansResponse.data.status === 'success') {
            ...
            // Create a map of managers and their clinicians
            const managerCliniciansMap = {};
            clinicians.forEach(clinician => {
              if (clinician.managerId) {
                if (!managerCliniciansMap[clinician.managerId]) {
                  managerCliniciansMap[clinician.managerId] = [];
                }
                managerCliniciansMap[clinician.managerId].push(clinician);
              }
            });
            ...
            const hierarchyStructure = {
              ...
              children: managers.map(manager => ({
                ...
                email: manager.email,
                image: manager.image,
                children: (managerCliniciansMap[manager._id] || []).map(clinician => ({
                  id: clinician._id,
                  name: clinician.name,
                  title: 'Doctor',
