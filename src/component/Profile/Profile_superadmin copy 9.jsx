  ...
  return (
    <Paper className={classes.hierarchyCard} elevation={0}>
      ...
      <div className={classes.hierarchyContainer}>
        ...
        {/* Assistants Level */}
        <div className={classes.assistantsGrid}>
          {assistants.map((assistant, index) => (
            <motion.div
              key={assistant._id}
              className={classes.assistantColumn}
              initial={{ y: 20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
            ...
            >
              ...
              {/* Clinicians for this Assistant */}
              <div className={classes.cliniciansContainer}>
                {clinicians
                  .filter(clinician => clinician.assistantId === assistant._id)
                  .map((clinician, cIndex) => (
                    <motion.div
                      key={clinician._id}
                      initial={{ y: 20, opacity: 0 }}
                      animate={{ y: 0, opacity: 1 }}
                      transition={{ duration: 0.5, delay: 0.2 + (0.1 * cIndex) }}
                    >
                      <UserCard
                        user={clinician}
                        role={clinician.specializedIn || "Clinician"}
                        color="#059669"
                      />
                    </motion.div>
                  ))}
