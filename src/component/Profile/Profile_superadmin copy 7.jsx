...
const TreeNodeContent = ({ 
  ...
  return (
    <Box className={classes.treeNodeContent}>
      <Box sx={{ 
        display: 'flex', 
        alignItems: 'center', 
        gap: 2,
  ...
  return (
    <Paper className={classes.hierarchyCard} elevation={0}>
      ...
      <Fade in timeout={1000}>
        <Box className={classes.hierarchyTree}>
          <TreeView defaultExpanded={['admin', 'assistants', 'clinicians']}>
            {/* Admin Level */}
            ...
            <TreeItem 
              ...
              label={
                <TreeNodeContent
                  ...
                  color="#2563eb"
                  canAdd={true}
                  onAdd={() => setAddAssistantDialog(true)}
                  addButtonLabel="Assistant"
                />
              }
            >
              {/* Assistants Level */}
              <Typography className={classes.levelLabel}>
                Level 2: Assistants ({assistants.length})
              </Typography>
              {assistants.map((assistant) => (
                <TreeItem
                  key={assistant._id}
                  nodeId={`assistant-${assistant._id}`}
                  label={
                    <TreeNodeContent
                      name={assistant.name}
                      ...
                      onAdd={() => {
                        ...
                        setAddClinicianDialog(true);
                      }}
                      addButtonLabel="Clinician"
                    />
                  }
                >
                  {/* Clinicians Level */}
                  <Typography className={classes.levelLabel}>
                    Level 3: Clinicians ({clinicians.filter(c => c.assistantId === assistant._id).length})
                  </Typography>
                  {clinicians
                    .filter(clinician => clinician.assistantId === assistant._id)
                    .map((clinician) => (
                      <TreeItem
                        key={clinician._id}
                        nodeId={`clinician-${clinician._id}`}
                        label={
                          <TreeNodeContent
                            name={clinician.name}
