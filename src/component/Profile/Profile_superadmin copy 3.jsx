...
const ImageUploadModal = styled(Dialog)(({ theme }) => ({
  '& .MuiBackdrop-root': {
...
const ProfileHeader = ({ adminData, classes, setEditModalOpen }) => {
  ...
  const handleImageUpload = async () => {
    ...
    if (!adminPortal || !selectedImage) {
      toast.error('Unauthorized or no image selected');
      return;
    }
    ...
    try {
      ...
      formData.append('companyImage', selectedImage);

      const response = await axios.patch(
        'https://rough-1-gcic.onrender.com/api/admin/update-media',
        formData,
        {
          headers: {
      ...
      if (response.data.status === 'success') {
        ...
        // Refresh the page or update the image state
    ...
    >
      ...
      >
        <StyledDialogTitle>
          ...
          <span className="title-text">Update Company Image</span>
        </StyledDialogTitle>
        ...
        <StyledDialogContent>
          ...
          >
            ...
            <UploadIcon
              ...
              transition={{ type: "spring", stiffness: 300 }}
            >
              <Icon path={mdiCloudUpload} size={1.5} color="white" />
            </UploadIcon>
            <Typography 
              variant="h6" 
              sx={{ 
