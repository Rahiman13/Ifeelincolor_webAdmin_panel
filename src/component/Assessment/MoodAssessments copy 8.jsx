    ...
    if (adminPortal === 'true' && token) {
      try {
        ...
        if (newMoodAssessment.type === 'mcq') {
          ...
          requestData.mcqOptions = newMoodAssessment.mcqOptions.map(opt => ({
            text: opt.text,
            isCorrect: opt.isCorrect
          }));
        } else {
    ...
    try {
      ...
      if (adminPortal === 'true' && token) {
        ...
        if (selectedMoodAssessment.type === 'mcq') {
          if (selectedMoodAssessment.mcqOptions.length === 0) {
          ...
          if (selectedMoodAssessment.mcqOptions.some(opt => !opt.text.trim())) {
        ...
        if (response.data.status === 'success') {
          ...
          fetchMoodAssessments(); // Refresh the list
        }
      }
    } catch (error) {
      console.error('Error updating assessment:', error);
