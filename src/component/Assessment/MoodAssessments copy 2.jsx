...
export default function MoodAssessments() {
  ...
  const handleMoodAssessmentModalOpen = async (mode, assessmentId) => {
    ...
    if (mode === 'edit' && assessmentId) {
      await fetchMoodAssessmentDetails(assessmentId);
    }
  ...
  const handleSaveNewMoodAssessment = async () => {
    ...
    if (adminPortal === 'true' && token) {
      try {
        ...
        formData.append('score', newMoodAssessment.score);
        ...
        if (newMoodAssessment.type === 'mcq') {
          ...
          formData.append('mcqOptions', JSON.stringify(newMoodAssessment.mcqOptions));
        } else {
          formData.append('answer', newMoodAssessment.answer);
        }
    ...
    if (adminPortal === 'true' && token && selectedMoodAssessment) {
      try {
        ...
        let assessmentData = {
          ...
          score: selectedMoodAssessment.score,
          category: selectedMoodAssessment.category,
          media: selectedMoodAssessment.media,
        };

        // For MCQ, ensure the answer is the correct option
        if (selectedMoodAssessment.type === 'mcq') {
          const correctOption = selectedMoodAssessment.mcqOptions.find(option => option.isCorrect);
          if (!correctOption) {
            toast.error('Please select a correct answer for MCQ');
            setActionLoading(false);
            return;
          }
          assessmentData.answer = correctOption.text;
          assessmentData.mcqOptions = selectedMoodAssessment.mcqOptions;
        }

        console.log('Sending updated assessment data:', assessmentData);
        ...
        if (error.response) {
          ...
          console.error('Error response status:', error.response.status);
          console.error('Error response headers:', error.response.headers);
          toast.error(`Error updating assessment: ${error.response.data.message || 'Unknown error'}`);
        } else if (error.request) {
          console.error('Error request:', error.request);
                ...
                <MoodAssessmentCard>
                  <CardContent>
                    <Box display="flex" justifyContent="space-between" alignItems="center">
                      ...
                      <Box>
                        <IconButton onClick={() => handleMoodAssessmentModalOpen('view', assessment._id)}>
                          <Visibility sx={{ color: '#fff' }} />
                        </IconButton>
                        <IconButton onClick={() => handleMoodAssessmentModalOpen('edit', assessment._id)}>
                          <Edit sx={{ color: '#fff' }} />
                        </IconButton>
                        <IconButton onClick={() => handleDeleteMoodAssessment(assessment._id)}>
                          <Delete sx={{ color: '#fff' }} />
                        </IconButton>
                      </Box>
                    </Box>
      ...
      >
        ...
        <DialogContent>
          <TextField
            ...
            onChange={(e) => setSelectedMoodAssessment({ ...selectedMoodAssessment, question: e.target.value })}
            disabled={modalMode === 'view'}
            sx={{ mb: 2 }}
          />
          <Select
            ...
            onChange={(e) => setSelectedMoodAssessment({ ...selectedMoodAssessment, type: e.target.value })}
            disabled={modalMode === 'view'}
            sx={{ mb: 2 }}
          >
          ...
          <TextField
            ...
            onChange={(e) => setSelectedMoodAssessment({ ...selectedMoodAssessment, score: e.target.value })}
            disabled={modalMode === 'view'}
            sx={{ mb: 2 }}
          />
          <Select
            ...
            onChange={(e) => setSelectedMoodAssessment({ ...selectedMoodAssessment, category: e.target.value })}
            disabled={modalMode === 'view'}
            sx={{ mb: 2 }}
          >
            <MenuItem value="" disabled>Select Mood</MenuItem>
          ...
          <TextField
            ...
            label="Media URL"
            name="media"
            value={selectedMoodAssessment?.media || ''}
            onChange={(e) => setSelectedMoodAssessment({ ...selectedMoodAssessment, media: e.target.value })}
            disabled={modalMode === 'view'}
            sx={{ mb: 2 }}
          />
          {selectedMoodAssessment?.type === 'blanks' && (
            <TextField
              ...
              onChange={(e) => setSelectedMoodAssessment({ ...selectedMoodAssessment, answer: e.target.value })}
              disabled={modalMode === 'view'}
              sx={{ mb: 2 }}
            />
          )}
                ...
                <Box key={index} display="flex" alignItems="center" mt={2}>
                  <TextField
                    ...
                    onChange={(e) => {
                      ...
                      setSelectedMoodAssessment({ ...selectedMoodAssessment, mcqOptions: updatedOptions });
                    }}
                    disabled={modalMode === 'view'}
                  />
                  <FormControlLabel
                    control={
                      <Checkbox
                        ...
                        onChange={(e) => {
                          const updatedOptions = selectedMoodAssessment.mcqOptions.map((opt, i) => ({
                            ...opt,
                ...
                <Button
                  onClick={() => {
                    ...
                    setSelectedMoodAssessment({
                      ...
                      mcqOptions: [...selectedMoodAssessment.mcqOptions, newOption]
                    });
                  }}
                  startIcon={<Add />}
                  sx={{ mt: 2, backgroundColor: '#007bff', color: 'white', '&:hover': { backgroundColor: '#0056b3' } }}
        ...
        <DialogActions>
          <Button onClick={handleMoodAssessmentModalClose}>Cancel</Button>
          {modalMode === 'edit' && (
