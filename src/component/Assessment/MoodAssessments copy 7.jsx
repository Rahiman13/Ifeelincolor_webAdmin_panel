  ...
  const handleSaveNewMoodAssessment = async () => {
    ...
    if (adminPortal === 'true' && token) {
      try {
        ...
        formData.append('score', newMoodAssessment.score.toString());
    ...
    if (adminPortal === 'true' && token && selectedMoodAssessment) {
      try {
        ...
        const requestData = {
          ...
          category: selectedMoodAssessment.category._id,
          answer: selectedMoodAssessment.answer
        };

        if (selectedMoodAssessment.type === 'mcq') {
          requestData.mcqOptions = selectedMoodAssessment.mcqOptions;
        }

        let finalData = requestData;
        if (selectedMoodAssessment.type === 'blanks' && selectedFile) {
          const formData = new FormData();
      ...
      >
        ...
        <DialogContent>
          ...
          <FormControl fullWidth margin="normal">
            ...
            <Select
              value={selectedMoodAssessment.category?._id || ''}
              name="category"
              onChange={handleMoodAssessmentChange}
              label="Category"
            >
          ...
          {selectedMoodAssessment.type === 'blanks' && (
            <Box sx={{ mt: 3 }}>
              ...
              <Button
                ...
                startIcon={<Upload />}
              >
                {selectedMoodAssessment.media ? 'Change Media' : 'Upload Media'}
              </Button>
            </Box>
          )}
          ...
          <Box sx={{ mt: 3, p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
            <Typography variant="subtitle2" color="textSecondary">
              Assessment ID: {selectedMoodAssessment._id}
            </Typography>
            <Typography variant="subtitle2" color="textSecondary">
              Category Model: {selectedMoodAssessment.categoryModel}
            </Typography>
            <Typography variant="subtitle2" color="textSecondary">
              Version: {selectedMoodAssessment.__v}
            </Typography>
          </Box>
        </DialogContent>

        <DialogActions>
          <Button onClick={handleMoodAssessmentModalClose}>
            Cancel
          </Button>
          <Button 
