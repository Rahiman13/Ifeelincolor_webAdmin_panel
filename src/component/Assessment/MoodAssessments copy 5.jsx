...
import {
  ...
  Checkbox, FormControlLabel, ToggleButton, ToggleButtonGroup, Chip
} from '@mui/material';
import { styled } from '@mui/material/styles';
import { Add, MoreVert, Visibility, Edit, Delete, ExpandMore, ExpandLess, Assessment, DeleteOutline } from '@mui/icons-material';
...
export default function MoodAssessments() {
  ...
  const handleMoodAssessmentModalOpen = async (mode, assessment) => {
    ...
    if (mode === 'edit' && assessment) {
      try {
        const token = sessionStorage.getItem('token');
        const response = await axios.get(`https://rough-1-gcic.onrender.com/api/test/${assessment._id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });

        if (response.data.status === 'success') {
          // Ensure mcqOptions exists for MCQ type
          const assessmentData = response.data.body;
          if (assessmentData.type === 'mcq' && !assessmentData.mcqOptions) {
            assessmentData.mcqOptions = [{ text: '', isCorrect: false }];
          }
          setSelectedMoodAssessment(assessmentData);
        }
      } catch (error) {
    ...
    if (adminPortal === 'true' && token) {
      try {
        ...
        const assessmentData = {
          ...
          media: newMoodAssessment.media,  // Add media field
          mcqOptions: newMoodAssessment.type === 'mcq' ? newMoodAssessment.mcqOptions : undefined
        };

        const response = await axios.post(
          'https://rough-1-gcic.onrender.com/api/test/create',
          assessmentData,
          {
            headers: { Authorization: `Bearer ${token}` }
          }
  ...
  const handleEditMoodAssessment = async () => {
    ...
    if (adminPortal === 'true' && token && selectedMoodAssessment) {
      try {
        ...
        // Validate required fields
        if (!selectedMoodAssessment.question || !selectedMoodAssessment.type || !selectedMoodAssessment.category) {
          toast.error('Please fill in all required fields');
          return;
        }
        ...
        if (selectedMoodAssessment.type === 'mcq') {
          if (!selectedMoodAssessment.mcqOptions?.length) {
            toast.error('Please add at least one option for MCQ');
            return;
          }
          if (!selectedMoodAssessment.mcqOptions.some(opt => opt.isCorrect)) {
        ...
        const updateData = {
          ...
          score: Number(selectedMoodAssessment.score),
          category: selectedMoodAssessment.category,
          answer: selectedMoodAssessment.answer,
          media: selectedMoodAssessment.media,  // Add media field
        ...
        console.error('Error updating assessment:', error);
        toast.error(error.response?.data?.message || 'Error updating assessment');
      } finally {
        setActionLoading(false);
      }
    }
